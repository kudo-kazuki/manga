...
const results = []
const add = (ok, name, detail, remedy) =>
    results.push({ ok, name, detail, remedy })
const hasPlaywright = fs.existsSync(PLAYWRIGHT_ENTRY)
add(
    hasPlaywright,
    'playwright',
    hasPlaywright ? PLAYWRIGHT_ENTRY : 'frontend/node_modules にありません',
...
if (hasPlaywright) {
    try {
        chromiumPath = (
            await import(pathToFileURL(PLAYWRIGHT_ENTRY).href)
...
add(
    Boolean(chromiumPath && fs.existsSync(chromiumPath)),
    'Chromium',
    chromiumPath && fs.existsSync(chromiumPath)
        ? chromiumPath
        : 'インストールされていません',
    `cd ${FRONTEND_ROOT} && npx playwright install chromium`,
)
...
try {
    ...
    responseDetail = `${BASE} → HTTP ${response.status}`
} catch {
    /* 診断結果に反映する */
}
...
if (failed.length) {
    console.log('\n不足している前提（実行にはユーザー許可が必要）:')
    for (const result of failed)
        console.log(`- ${result.name}: ${result.remedy}`)
    process.exitCode = 1
} else
    console.log(
