...
export const OUT =
    process.env.BROWSER_CHECK_OUT ??
    path.join(os.tmpdir(), 'manga-browser-check')
fs.mkdirSync(OUT, { recursive: true })
export const outFile = (name) => {
    ...
    if (
        ...
        throw new Error(`出力先が一時ディレクトリの外になります: ${name}`)
    return file
}
export const loadChromium = async () => {
    if (!fs.existsSync(PLAYWRIGHT_ENTRY))
        throw new Error(
            `playwright がありません: ${PLAYWRIGHT_ENTRY}\n  → cd ${FRONTEND_ROOT} && npm install --save-dev playwright`,
        )
    return (await import(pathToFileURL(PLAYWRIGHT_ENTRY).href)).chromium
}
const attachDiagnostics = (page) => {
    ...
    page.on('pageerror', (error) =>
        report('PAGEERROR', error.message.slice(0, 300)),
    )
    page.on('console', (message) => {
        if (message.type() === 'error')
            report('CONSOLE', message.text().slice(0, 300))
    })
}
    ...
    page.evaluate(
        ({ quiet, limit }) =>
            new Promise((resolve) => {
                ...
                const deadline = setTimeout(() => done(false), limit)
...
export const visit = async (
    ...
    if (!urlPath.startsWith('/'))
        throw new Error(`パスは / で始めてください: ${urlPath}`)
    await page.goto(new URL(urlPath, BASE).toString(), {
    ...
    if (settle) {
        try {
            if (!(await waitForSettled(page)))
                console.log(
                    'waitForSettled: DOM の変化が止まりませんでした（続行）',
                )
        } catch (error) {
...
export const shot = async (page, name, { fullPage = false } = {}) => {
    const file = outFile(`${name}.png`)
